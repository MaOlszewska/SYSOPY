...
void bake_pizza(){
    ...
    ret_buf.sem_op = 1;
    ret_buf.sem_flg = 0;
    // zwraca identyfikator do segmentu pamięci wspólnej
    ...
    while(baked == 0){
        // wykonuje operacje na zbiorze semaforów
        semop(table_sem, &sops, 1);
        // dołaćza segment pamieci wspólnej do przestrzeni adresowej
        ...
        // odłączenie segemntu po zakonczeiu pracy
        shmdt(t);
        //wykonujer operacje na zbiorze semaforów
...
void finish_baking(){
    ...
    oven_shm = shmget(oven_shm_key, 0 ,0 );
    while(baked == 0){
        // wykonywanie operacje na zbiorze samforów
        semop(table_sem, &sops, 1);
        semop(oven_sem, &sops, 1);

        //dołącza segemnt pamieci wspólnej do przestrzeni adresowej
        table* t = shmat(table_shm, NULL, 0);
        oven* o = shmat(oven_shm, NULL, 0);
        if (t->pizza > 0){
            ...
            o->pizza -- ;
            // odłącza segment po zakonczonej pracy
            shmdt(o);
